...
void Usage(void)
{
   ...
   fprintf(stderr,
	   "  options:                                                                   [  def][ min--max]\n");
   fprintf(stderr,
	   "    -im mgc        : input file for spectrum                                 [  N/A]\n");
   fprintf(stderr,
	   "    -if lf0        : input file for Log F0                                   [  N/A]\n");
   fprintf(stderr,
	   "    -or raw        : filename of output raw audio (generated speech)         [  N/A]\n");
   fprintf(stderr,
	   "    -ow wav        : filename of output wav audio (generated speech)         [  N/A]\n");
   fprintf(stderr,
	   "    -m  i          : order of MGC/LSP                                        [   24][   1--]\n");
   fprintf(stderr,
   ...
   fprintf(stderr,
	   "    -l             : regard input as log gain (LSP)                          [  N/A]\n");
   fprintf(stderr,
	   "    -t             : regard input as STRAIGHT spectrum                       [  N/A]\n");
   fprintf(stderr,
	   "    -h             : print this help                                         [  N/A]\n");
   fprintf(stderr, "  note:\n");
...
int main(int argc, char **argv)
{
   ...
   MGC_Vocoder_initialize(&v, order, stage, use_log_gain, sampling_rate,
			  fperiod);
   ...
   for (i = 0; i < total_frame; i++) {
	 ...
	 spectrum[j] = (double) ftemp;
      ...
      if (use_straight)
	 spectrum[0] += 10.0;
      MGC_Vocoder_synthesize(&v, order, lf0, spectrum, alpha, beta,
			     &rawdata[i * fperiod]);
   }
   MGC_Vocoder_clear(&v);
   if (rawfp)
